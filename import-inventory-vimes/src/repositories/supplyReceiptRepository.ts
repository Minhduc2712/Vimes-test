...
import type {ISupplyReceiptRepository, SupplyReceiptDetailRow, SupplyReceiptRow, SupplyReceiptWithDetails} from '../interfaces/supplyReceipt.js';
import type {SupplyReceipt} from '../models/SupplyReceipt.js';

export class SupplyReceiptRepository implements ISupplyReceiptRepository {
    async list(): Promise<SupplyReceiptRow[]> {
        const result = await pool.query<SupplyReceiptRow>('SELECT * FROM supply_receipts WHERE is_deleted = FALSE ORDER BY id DESC');
        return result.rows;
    }

    async getById(id: number): Promise<SupplyReceiptWithDetails | null> {
        const header = await pool.query<SupplyReceiptRow>('SELECT * FROM supply_receipts WHERE id = $1 AND is_deleted = FALSE', [id]);
        const headerRow = header.rows[0];
        if (!headerRow) return null;

        const details = await pool.query<SupplyReceiptDetailRow>('SELECT * FROM supply_receipts_details WHERE receipt_id = $1 AND is_deleted = FALSE ORDER BY id', [id]);
    ...
    async create(receipt: SupplyReceipt): Promise<SupplyReceiptWithDetails> {
        ...
        try {
            ...
            const insertedHeader = await client.query<SupplyReceiptRow>(
            ...
            let insertedDetails: SupplyReceiptDetailRow[] = [];
            ...
            if (detailsInput.length > 0) {
                ...
                const insertedDetailsResult = await client.query<SupplyReceiptDetailRow>(
                    `INSERT INTO supply_receipts_details (
                        receipt_id,
                        item_id,
                        doc_quantity,
                        actual_quantity,
